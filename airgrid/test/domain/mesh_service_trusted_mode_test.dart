...
void main() {
  ...
  test(
    'trusted-only: sendPrivateMessage returns notTrusted for untrusted peer',
    () async {
      final transport = FakeTransport();
      final identity = await _identity();
      final contactStore = InMemoryKnownContactStore();
  ...
  test(
    'trusted-only: sendPrivateMessageToContact returns notTrusted',
    () async {
      final transport = FakeTransport();
      final identity = await _identity();
      final contactStore = InMemoryKnownContactStore();
  ...
  test(
    'everyoneNearby: accepts inbound location from non-trusted sender',
    () async {
      final transport = FakeTransport();
      final identity = await _identity();
      final mesh = AirGridMeshService(
      ...
      transport.receiveBytes(
        'ep-a',
        TransportCodec.encode(_locationPacket(sender)),
      );
      await Future<void>.delayed(Duration.zero);
